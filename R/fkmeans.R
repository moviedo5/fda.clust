...
# @param max.comb Maximum number of initial selection of centers (only used when \code{method="exact"}).
...
#' 
#' @export
fkmeans <- function(fdataobj,ncl=2,metric=metric.lp
...
while ((i<max.iter) & (!same_centers)) {
  iterar <- FALSE
 out3 <- kmeans_groups(out1,draw=draw)
...
#else{
#     # print("2. update")
     out2 <- kmeans_update(out1, group=out3$cluster
